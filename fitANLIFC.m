...
% are omitted in this reference implementation.
    ...
    % ---------- Antecedent init: FCM (centers C and per-feature sigmas) ----------
    if exist('fcm','file') == 2
        [C, U] = fcm(X, K, fcmOptions);     % C: K×D, U: K×N
        centers = C;
        sigmas  = zeros(K, D);
        for k = 1:K
            wk = U(k,:);
            wk = wk / (sum(wk) + tiny);
            dif2 = bsxfun(@minus, X, centers(k,:)).^2;   % N×D
            sigmas(k,:) = sqrt( max(1e-8, 0.5*(wk*dif2)) );
        end
    else
        warning('FCM not found; using kmeans init.');
        [idx, C] = kmeans(X, K, 'MaxIter',200, 'Replicates',1);
        centers = C;
        sigmas  = zeros(K,D);
        for k = 1:K
            Xk = X(idx==k,:);
            if isempty(Xk), Xk = X; end
            dif2 = bsxfun(@minus, Xk, centers(k,:)).^2;
            sigmas(k,:) = sqrt( max(1e-8, 0.5*mean(dif2,1)) );
        end
    end
